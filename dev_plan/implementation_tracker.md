...
| T30 | Selection (M10) | review | claude (main) | T13, T23 |
...
| T30 | **human** | **Day highlights are poor, and the cause is the disabled face signal.** Reviewing them: a hotel doorway from a tram, a street of parked vans, four museum paintings, two near-identical façades — and no cathedral, no Golden Hall, nobody's family. With `models.face_detector_model` unset, `face_count` is NULL for all 277 photos and quality is sharpness+exposure+contrast only, so a flat well-lit façade outscores a portrait. **Configuring a YuNet model is the single highest-value change available.** | OPEN — needs a decision |
| T30 | T13 | Technical quality ≠ narrative importance, as P02's reviewer said. Even with faces enabled, "the concert we came for" is not a measurable image property. Density weighting helps; the rest is chapters and human overrides. | open — accepted limitation |
