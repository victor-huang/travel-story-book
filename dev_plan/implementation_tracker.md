...
| P07 | T31/T41 | **The ids stayed accurate and the prose drifted.** The model's `trip_summary.md` places the Pestsäule on 20 July; `story.json` correctly has it in `2026-07-18-ch04`. Same model, same run: structured output anchored by asset ids could not wander, free prose had nothing holding it. Argues for `story.json` as the rendering source of truth. | resolved — informs T44 |
