...
| T45 | T13 | The two screen captures were classified `screenshot` correctly — the classifier was right and the *scope* of `reject_content_classes` was the gap: it governs highlight eligibility only. Whether rejected content classes should also leave the artifact by default is a policy question affecting every user, so it is the human's call rather than a silent change. | open — needs a decision |
