...
| T44 | Story round trip: ingest `story.json` + model-written context | done | claude (main) | T40, T41 |
...
| --- | --- | --- | --- |
| **iOS I17 / device run** | owner of `src/story_book/pipeline/metadata.py` | **Nothing reads the sidecars the phone writes, and on a real library that loses the capture time of roughly a third of the assets.** Measured on a first device export (8 assets from a real iPhone library): 3 carry **no embedded timestamp at all** — two are images with no EXIF beyond dimensions (saved or screenshotted, not camera captures) and one is an `.mov` whose QuickTime `CreateDate` is the `0000:00:00` sentinel. The pipeline correctly reports `taken_local: null` and they are placed on no day. **PhotoKit knew the answer for all three**, and `FolderWriter` already wrote it to `<filename>.sidecar.json` with `creation.source: "photokit_asset"` — the file sits beside the media and nothing opens it. Request: `MetadataStage` reads a sidecar when present and uses it as a **fallback**, never an override — EXIF carries a local wall time *and* an offset, which is strictly richer than the sidecar's bare UTC instant — and records which source won, in the manner of `gps_source` distinguishing a measurement from a derivation. Adding a column means bumping that stage's `version`, per the `exif_local` lesson. **Blocks nothing today; caps the quality of every hosted trip until done.** | open — filed 2026-08-10 |
| **iOS I16** | owner of `src/story_book/pipeline/events.py` (event sequencing) | **`event_id` is not stable when two items share a `taken_utc`.** Found by the iOS parity harness: `tz_before_1.jpg` (23:10+02:00) and `tz_after_1.jpg` (00:10+03:00) are both 21:10 UTC, and they exchange `#2` and `#3` between two libraries holding the same logical content — the ordering tie is broken by something byte-dependent. Deterministic for identical bytes, so the build-twice test cannot see it; not stable across a re-encode, which is exactly the iOS case. `trip.json`'s own rule is that every published id is a function of the media set, and a tie makes `<date>#<seq>` arbitrary. Suggested fix: break the tie on `filename`, which `overrides.toml` already treats as an address, so the order becomes a function of the media set rather than of its encoding. `tests/ios_parity/test_trip_structure_parity.py::TestKnownPermutation` pins the current behaviour and will fail when this is fixed, which is the prompt to update it. **Does not block iOS work** — the harness compares boundaries as a multiset. | open — filed 2026-08-09 |
| **iOS I25** | owner of `src/story_book/export/report.py` | **Make `MEDIA_REL_FROM_INDEX` / `MEDIA_REL_FROM_DAY` overridable.** They are module constants at `report.py:56-57`, read at `:454` and `:472`. The iOS app renders the same report with `media_rel="storyasset://"` and resolves each request through a custom `WKURLSchemeHandler` against the phone's originals (iOS tracker D4/I25). The templates already interpolate `media_rel` (`templates/_shot.html:5`, `templates/index.html:32`), so this is a parameter on the render entry point, not a template change and not string surgery on generated HTML. Default must stay byte-identical — a fork of the renderer is the thing this avoids. **Blocks iOS I25.** | **resolved 2026-08-09** with the human's authorisation. `MediaPrefix` is a frozen dataclass parameter on `render_report`, defaulting to the existing constants; `MediaPrefix.absolute("storyasset://")` is the app's case. The load-bearing test asserts the default prefixes *literally* — the first version compared the default against itself and passed with the default deliberately broken to `./`, which is a test that cannot fail. 1772 tests green. |
...
| P07 | T11/T12 | **A shipped upgrade bug, found while re-checking the above.** `media.exif_local` was added to stop the timezone stage reading a field it also writes, but `MetadataStage.version` stayed at 1 — so on any library built before that commit the column stays NULL forever, metadata never re-runs, and the drift the column exists to prevent continues. The reviewed packages were built on such a DB and carried a 00:59 stop that never happened; a fresh build has 3 stops on 19 July, not 6. Fixed by bumping the stage to v2, with tests for the upgrade path. **Adding a column a stage writes means bumping that stage's version.** | resolved |
| P07 | **human** + T41 | **First real journal generated. The stable-id architecture is validated: 56/56 asset references resolved, 100% coverage, nothing dangling.** Honesty instructions were followed — visual-identification caveats on landmarks, unphotographed stops named not invented, and the storyboard declining exact ranges pending proxy review. | resolved |
| P07 | **human** | **A reported prose error was not one — my mistake, corrected.** I claimed the generated summary moved the Pestsäule two days. The traveller was in the old town again on 20 July and `IMG_2072` (14:13) is a column monument; `story.json` makes the same claim in a caption, so there was no divergence to reason from. I judged a claim about a photograph from metadata without opening the photograph. | resolved — claim withdrawn |
| T45 | — | **`reject` now removes an item from the artifact, not just from the highlights.** The example file said "never include these" while the implementation only blocked selection, so two screen captures kept counting toward the day, dropping map pins and forming a phantom 00:59 stop. A rejected item leaves `trip.json`; an event left with no members disappears with it; `privacy.excluded_by_override` counts what a human removed so a shrunken total is visibly a decision. The file stays in the library and the DB. | resolved |
| T45 | T13 | **Screenshots now leave the artifact by default**, via `timeline.exclude_content_classes = ["screenshot"]`. Distinct from `quality.reject_content_classes`, which only removes highlight eligibility -- a screenshot that is merely ineligible still counts toward the day, pins the map, and can be the sole member of a stop. Receipts and documents are **kept**: a receipt is a photograph of something physically in front of you, which is weak evidence of a place but still evidence, where a screenshot is a picture of a phone. A pinned photo survives the filter, as it survives the quality floor. On the real trip: 6 screenshots dropped, two further phantom single-screenshot stops on 19 July gone. | resolved |
| T47 | T44 | **`<out>/story/` holds what a chat returned**, discovered automatically by `report` and `package` so the flags become optional. Anchored to `--out`, never the cwd. It is the only content in the output directory a rebuild cannot recreate, so it gets its own directory and a README saying so — mixing irreplaceable input into a directory documented as disposable is a trap. Two tests assert neither `report` nor `package` touches it. | resolved |
| T46 | T40 | **The day map is now Leaflet over OSM tiles, reversing an earlier call.** The SVG-only map showed a route with no streets, which is not what a reader wants from a travel book. The original argument — tiles fail offline, and grey rectangles beat no map — was overstated: Leaflet still draws the route and markers without tiles. Leaflet is vendored so no *code* comes from a network; the SVG is kept as a `<noscript>` fallback. Two offline tests were rewritten: both kept passing after the change while saying nothing true, one because it only checked the index and one because its regex could not see the tile URL inside a JSON block. | resolved |
| T44 | — | **The round trip closes.** `story-book report --story story.json` renders the model's day narratives, chapters and captions over the existing report; `--context` accepts the YAML a model actually emits. The story is an *overlay* — words only, never structure — so a chapter citing media the trip does not contain is dropped rather than inventing a page, and `check-story` reports it. | resolved |
| T44 | T33 | Trip context is now lenient about **unknown top-level sections** and unknown *traveler* fields (warned, not refused): that is where a model puts its own bookkeeping, and losing a build over `context_id` would be absurd. Still strict on the settings this tool owns, where a typo means a deliberate choice silently did nothing. `display_name` accepted as `name`. | resolved |
| T44 | T41 | `schema/trip_context.template.toml` now ships in every package and the prompt asks for the context back in that shape — the same lesson as `story.schema.json`: publish the target rather than write an adapter that guesses. | resolved |
| P07 | T41 | What survives: the model hedged the unconfirmed *attraction* but asserted the *column* flatly, though both are inferences from an image. The prompt asks for that flag. Not catchable by `check-story` — both are structurally valid — so it stays a prompt-discipline issue. | open — prompt wording |
| P07 | T41 | **The response contract was not followed:** 13 chapters with zero `source_event_ids`, `video_scenes`/`uncertainties` renamed, `layout_pages` and `requested_additional_context` absent. Richer than asked for, and unreadable by a renderer built to the contract. Fixed by publishing `schema/story.schema.json` inside every package, naming it in the prompt, and adding `story-book check-story` which checks shape *and* grounding separately. | resolved |
| P06 | T41 | **The exported videos were JPEGs under `.mov` names.** `kind: "video"` with `export_path: ".../IMG_1815.mov"` and a still image on disk — the package advertised nine playable clips and shipped nine posters. Fixed with two honest modes: default ships `<id>_poster.jpg` as `image/jpeg` with `video_proxies_included: false` and a prompt that calls source ranges estimates; `--video-proxies` transcodes real 720p MP4s and switches the prompt to "watch them". A failed transcode falls back to the poster and says so. | resolved |
| P06 | T41 | Short clips (<2s, config `video.short_clip_max_seconds`) are marked `subtype: short_clip` / `storyboard_candidate: false` and ship one keyframe rather than five. Labelled by duration, not called a Live Photo — the files carry no `ContentIdentifier`. | resolved |
| P06 | T31 | After-midnight items carry `calendar_date` beside `day`, the trip states its `day_assignment_rule`, and the brief renders `00:59 (2026-07-20)`. A stop filed under 2026-07-19 with a calendar date of the 20th previously looked like a malformed timestamp. | resolved |
| P06 | T41 | Removed the duplicated top-level `pinned_by_human`; added numeric `duration_seconds` beside `duration_display`; trip bounds now carry offsets and UTC, ordered by `taken_utc` rather than by lexicographic comparison of offset-bearing local strings. | resolved |
| P06 | Phase 2 | Optional event *subclusters* — the reviewer marked them unnecessary for the MVP given that chapters are model-drawn. Revisit if AI chapter quality turns out to depend on them. | open — Phase 2 |
| P05 | T31/T41 | **The generated package was reviewed by ChatGPT: usable for journal writing, not yet ready to drive a renderer.** The gap was layout geometry and precise video segment references, not prose. Eight of ten requests implemented — offset-bearing timestamps at asset/day/trip level, captured-vs-included counts with `asset_scope`, every stop listed in the brief, keyframes with offsets plus a source-range output contract, frame geometry, 16-hex asset ids, `event_type: detected_cluster` with model-drawn chapters, selection reasons, a shipped JSON Schema, and `--zip` without `.DS_Store`. | resolved |
| P05 | T31 | **Adding offsets to `taken_local` exposed a latent bug:** event durations were computed from local wall time, which mixes offset-aware and naive strings in any library where some items resolved an offset and some did not — and gives the wrong answer across a zone change even when it does not raise. Durations now come from UTC instants. | resolved |
| P05 | T21 | Declined to emit a place `confidence`: the offline geocoder produces none, and inventing one is a fabricated measurement. Reports `source` and `precision: "city"` instead. If Nominatim is ever enabled it does return a confidence, and that is where the field should come from. | resolved — by declining |
| P05 | Phase 2 | Automatic video `highlight_ranges` (per-window motion and stability scoring, with a reason string) is genuinely new analysis rather than unexported data. Keyframes with offsets let a model pick a range; the automatic version is deferred rather than faked. | open — Phase 2 |
...
| T43 | **human** | Criterion 11 closed 2026-08-02: `config.home` set, filter ran over 274 items and excluded 0 — a checked result rather than a gap. Criterion 10 is met in substance (P07 produced a real journal); what remains is judging the *current* package, which only a human can do. | resolved |
...
| 2026-08-02 | T12 | **An item with no GPS still has its offset tag, and it was being discarded.** `_resolve_without_gps` took the zone from a device neighbour or config and read the wall time as local, never consulting `exif_offset_minutes`. On the real trip 11 paragliding clips carried `-07:00` with no GPS *and* no device id, so they sat at 02:37 -- six hours from anything else that day, which also left them too isolated for `gps_backfill` to place, which excluded them from every export. The tag now supplies the instant when it lands the item beside the rest of the trip and the wall reading does not. Deliberately one-directional: a genuinely nocturnal frame is never dragged into the day just because daytime is busier. | yes -- Module 2 |
| 2026-08-02 | T12 | **Offset-vs-GPS conflicts are arbitrated by neighbours, not by a fixed rule.** One symptom has two causes: the camera clock really was on another zone (tag holds the instant), or the clock was already local and only the tag is stale (wall reading holds it). On the real trip 8 frames were the first kind and 6 the second, so the old unconditional rule moved 6 photographs 9 hours -- one pair past midnight. Now both readings are computed and the one nearest a confident same-device photo wins. Also: **only a *measured* position counts as an item's own GPS.** `gps_backfill` runs later, so on a second build it had interpolated coordinates for items that had none, sending them down the GPS path and shifting them an hour -- the same source tree resolving to different times on re-run. `TimezoneStage.version` 1 -> 2. | yes -- Module 2 |
...
| Date | Who | Entry |
| --- | --- | --- |
| 2026-08-09 | claude | **Clips were being rendered from 720p proxies and enlarged to 1080p — 41% of the detail retained.** The traveller asked whether clips kept the original resolution; they did not. `resolve_clip_sources` preferred the package proxy, which exists to be *small enough to upload*, not to render from. Original first now, proxy as fallback, and `reel.json` reports any clip whose source is shorter than the frame. Measured on IMG_1815: sharpness (variance of Laplacian) 81.4 from the original against 33.5 from the proxy at the same 1080p. |
| 2026-08-09 | claude | **`-map 0:a?` broke on 58 of 69 real clips once originals were used.** A modern iPhone writes spatial audio as a second `apac` track that this ffmpeg has no decoder for, plus five `mebx` metadata streams; mapping *every* audio stream tried to decode it and failed with "no decoder found for: none". Only the first track is wanted: `-map 0:a:0?`. Proxies had hidden it entirely, because transcoding one picks a single stream by default — the failure only appeared when the reel started reading real camera files. |
| 2026-08-09 | claude | **Three corrections from watching the reels.** (1) **Day-derived captions are no longer burned in.** A model asked for 398 captions templated 350 of them — *"A moment from &lt;place&gt; during our &lt;day&gt; day"*, one string covering up to 32 photographs. Two phrasing-independent signals catch it: a caption attached to more than one asset is not a caption of any of them, and one containing its own day's title is derived from the day. 48 real descriptions survive, none dropped wrongly; cue counts fell 127→18, 160→29, 89→22, 48→6. (2) **Burned-in text shrinks to fit one line** before wrapping, down to 55% of the configured size — Chinese wraps sooner than English because every glyph is full-width. (3) **A closing card**: a mosaic of that reel's own stills, dimmed, with the closing line over it, on a grid chosen to fill exactly. |
| 2026-08-03 | claude | **Reels can be narrowed by date range and place, and the four-part cut of the real trip is rendered.** 22 days is ~13 min as one montage; `--from`/`--to`/`--place`/`--name` compose so the seams can follow the geography rather than arithmetic — a Salzburg→Vienna travel day needs both filters, since dates cannot split it and places alone cannot separate two visits to one city. Rendered: **Zillertal & Tyrol 6.1 min, Salzburg & the Lakes 7.7, Vienna 3.9, Munich 2.1**. Also: the reel now honours a story's `timeline_duration_seconds` over the length of the source range it cut from, plus a new `clip_max_seconds` ceiling — the story asked for 6.5 min of footage across 67 clips while its ranges implied 12.4, and how long a montage runs is a rendering decision, not an editorial one. |
| 2026-08-03 | claude | **Two reel bugs found by an interrupted render.** `mux_subtitles` used `-map 0`, which copies the input's existing subtitle streams — so re-rendering a reel under the same name *appended* a second track instead of replacing it, and the stale one carries no language tag, showing as a nameless duplicate in a player. Now maps video and audio explicitly. Separately, a killed render leaves a `.partial.mp4` that is never valid: it is now cleared before rendering, and ffmpeg exiting 0 while writing nothing raises a named `ReelError` instead of surfacing as a `FileNotFoundError` from a rename three frames away. |
| 2026-08-02 | claude | **`overrides.toml` now accepts asset ids as well as filenames.** The traveller was reviewing the report and reaching for the id it prints in accent colour rather than the small dim filename beside it — reasonable, and safe: an `asset_id` is a prefix of the content hash, so it is stable for exactly the reason a filename is. Cluster and event ids are still refused, since those are reassigned every run. Eight hex characters minimum so a short filename cannot be misread as a prefix, a filename match still wins over a prefix lookup, and an ambiguous prefix says "use more characters". |
| 2026-08-02 | claude | **The 11 GoPro paragliding clips now export — by fixing the cause, not overriding the privacy filter.** They were excluded for having no coordinates; the reason they had none was a 9-hour timestamp error. With no GPS and no device id their `-420` offset tag was thrown away entirely (`tz_source = config`), putting them at 02:37 instead of 11:37 — too isolated for `gps_backfill` to interpolate. Honouring the tag when it lands the item beside the rest of the trip fixed all 11: now 47.18N 11.87E in the Zillertal, `is_near_home = 0`, and present in `trip.json`. Items without coordinates fell from 13 to 2, and both remaining are isolated single frames the conservative rule correctly leaves alone. **1557 tests pass.** |
| 2026-08-02 | claude | **Timezone conflict resolution fixed, and a determinism bug found underneath it.** The traveller's import log showed 14 offset-vs-GPS conflicts; testing each against its own camera's neighbouring frames showed **6 of 15 were resolved wrongly** by the unconditional "trust the tag for the instant" rule -- IMG_1993 pushed from 15:59 to 00:59 the next day, between neighbours at 15:44 and 16:13. Now neighbour-arbitrated: **42 in sequence, 0 out of sequence** on the full 1193-item trip, up from 8/6/1. Fixing it surfaced a worse latent bug: timezones ran before `gps_backfill` on build 1 and after it on build 2, so interpolated coordinates changed 143 GoPro timestamps by an hour on re-run. Interpolated positions are no longer treated as an item's own GPS. **1549 tests pass**; the new tests were shown to fail against the old behaviour (5 of 9). |
| 2026-08-02 | claude | **`--day` now writes `trip.<date>.mp4`, not `trip.mp4`.** Found while writing up how to add more days: rendering five per-day reels in a row would have left only the fifth, silently. Manifests and subtitle sidecars follow the same stem. Day renders reuse the whole-trip segment cache, so they cost only their own title card. |
| 2026-08-02 | human | **Reel acceptance criterion 2 met: "it is worth watching."** That was the one criterion no test could reach — every other one checks correctness, this one asks whether the thing is any good. Wave 5's reel is now **7 of 7**. Reached in four review rounds on real output: music chosen by listening to four candidates, clip audio added on request and ducked 8.4 dB, Mandarin subtitles added and then enlarged to 1.4x. |
| 2026-08-02 | claude | **Burned-in subtitle size is now a setting** — `--subtitle-scale` / `reel.subtitle_scale`, a multiple of the default frame-height/26 (42 px at 1080p), plus `reel.subtitle_bottom_margin`. Scoped honestly: it affects `--burn-in` only, because a soft track is sized by the player, and passing it without `--burn-in` says so instead of silently doing nothing. Validated before any encoding so a typo does not cost a full render. Verified end to end rather than in the cue PNGs alone: the bottom-band difference against the clean reel grows with the scale. **1531 tests pass.** |
