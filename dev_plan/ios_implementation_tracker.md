...
| I18 | **Selection review grid** — see and deselect what a scope found | review | claude (2026-08-12) | I10, I17 |
...
| S06 | Auth — Apple and Google, per-user isolation | review | claude (2026-08-12) | S01 |
...
**Delivered 2026-08-12 → review, not done.** `resolve_principal`'s body is now real verification
instead of a believed header: `Authorization: Bearer <token>` is decoded unverified only far enough
to read `iss`, checked against an exact two-entry allow-list (`https://accounts.google.com` and the
bare `accounts.google.com` form real Google tokens have used historically, plus
`https://appleid.apple.com`), then verified for real via `jwt.PyJWKClient(jwks_url)
.get_signing_key_from_jwt(token)` and `jwt.decode(..., algorithms=["RS256"], audience=..., issuer=...)`
— one code path for both providers, since both are RS256-over-JWKS. `sub` (never email — Apple
only returns email on first login) becomes the index identity, `index.ensure_user(kind="google"|
"apple", value=sub)` unmodified from what S02 already anticipated. The `X-Story-Identity` dev header
still works, now gated by `Settings.allow_dev_identity_header` (default `True`, so today's flows and
every existing dev-header test keep passing unchanged) — **a deployment reachable beyond localhost
must set this `False`**, which is D14's condition for exposing the service at all, restated as an
enforceable flag rather than a promise in prose. Neither `index.py` nor `index_sqlite.py` needed
touching; `IDENTITY_KINDS` already had `"google"`/`"apple"` waiting.

**The downgrade path is the thing that actually had to be proven, not just built.** A bad or
unparseable bearer token never falls through to the dev header — checked by a test that sends both
a malformed-issuer bearer token *and* a valid `X-Story-Identity` header on the same request and
asserts 401, because a silent fallthrough there would let a rejected token buy dev-header access on
any deployment that forgot to also set `allow_dev_identity_header=False`. 14 new tests (service
suite 226 → 240): valid Google and bare-issuer-form Google, valid Apple, expired, wrong audience,
unlisted issuer (with and without the downgrade attempt), tampered signature (one byte flipped in
the token's own signature segment), provider-not-configured (Google configured, Apple's client id
left empty — must not verify against a missing audience), dev header allowed and disallowed, and
the literal acceptance criterion run against **verified** tokens rather than the dev header:
two `sub`s through a real `TestClient` and a real route, account A gets 404 on account B's trip and
sees only its own in the list — the same property `test_ingest.py::TestPerUserIsolation` already
proved for the dev-header path, now proved for the path that matters in production. Only the JWKS
network fetch is mocked (`PyJWKClient.get_signing_key_from_jwt`, via a real RSA keypair generated
once per test module and a real `jwt.encode`/`jwt.decode` round trip) — signature, audience, issuer
and expiry checking all run for real, no live network call, matching this project's `--no-cloud`
philosophy applied to a test. Service suite 240/240, root `tests/unit` still green (untouched, no
file under `src/story_book/` touched), `ruff check`/`ruff format` clean.

**What "review, not done" means here.** Every check above runs against synthetic tokens signed by a
test key — no live Google or Apple token from a real client has exercised this path, because that
needs a provisioned OAuth client id / Services ID and a device, which is I23's job (client half),
not this one's. And **open question 3 (account deletion vs. indefinite retention) is still
unresolved** — it blocks Apple *App Store submission*, not landing this code, so it is called out
here rather than silently dropped. Not marking I23 unblocked myself; that judgment belongs to
whoever picks it up, but S06 landing is what unblocks it.
...
| Date | Who | Entry |
| --- | --- | --- |
| 2026-08-13 | claude | **Requested directly ("let's create some tests for the feature and bug fixes we did") — and writing them found a real bug the manual tap-through never would have.** `ReelClient` (I30/I31/I33's own networking client, live-curl-verified but never unit tested) gets `Tests/StoryAppTests/ReelClientTests.swift`, 6 real tests executing today via `swift test` (hostless — `ReelClient`/`ReelOptions`/`ReelStart`/`ReelDownload`/`ReelManifest` all sit outside `#if os(iOS)`), against a self-contained stub `URLProtocol` (duplicated from `StoryServiceTests/StubHTTP.swift`'s shape rather than imported, matching I18's `SelectionSeeding` precedent — `StoryAppTests` has no package dependency on `StoryServiceTests`). Covers: every `ReelOptions` field reaching the wire exactly as set, a 422 before any job is queued, the full `GET /jobs/{id}/reel` response shape including the inline `reel_json` manifest read by dotted path, and — the pair that actually matters to `ReelViewerScreen` — a build's job id (404) told apart from a still-rendering reel (409), each the other's control. **The bug**: `ReelOptions`'s own doc comment claimed "an absent field is encoded as JSON `null` (`encodeIfPresent` is deliberately not used)," but no custom `encode(to:)` existed — Swift's synthesized `Encodable` for an `Optional` stored property calls `encodeIfPresent` by construction, so an unset option was silently *omitted* from the wire the whole time, exactly what the comment says does not happen. Functionally harmless today (the service's `ReelRequest` treats an absent key and an explicit `null` identically), but the code did not match its own stated contract, and a documentation claim nobody could verify is the same failure class this file's own rules keep naming. Added a hand-written `encode(to:)` so the code now does what the comment always said. Root hostless suite 226 → 232, all passing; physical-device build (`xcodebuild`) still clean, reinstalled. **What's not covered, and why, rather than silently skipped:** today's other fixes — the security-scope ordering in `MusicImporter`, the `AVAudioSession`/`AVPlayerViewController` configuration in `ReelPlayerView`, and `LoopModel`'s folder-scoped report/reel caching — are each untestable for a different real reason rather than left out for lack of time: the security-scope bug only manifests against a URL a real `.fileImporter`/`UIDocumentPickerViewController` handed out, which nothing in this suite can manufacture (same limit I31's own log entry already named); the audio/fullscreen configuration is `AVAudioSession`/`AVPlayerViewController` runtime state with no pure-logic seam to assert against; and `LoopModel`'s interesting state (`runsByFolder`, `RunRecord`, `loadRun`, `saveRun`, `saveReelJobID`) is `private` — invisible even to `@testable import` — and reading it from outside would mean writing directly to the real `UserDefaults.standard` under a hardcoded key string, which is brittle by construction and not something I did unasked. All three would need either a real device/simulator tap-through (already how they were found and fixed today) or a small testability refactor (dependency-injected `UserDefaults`, or an Xcode-native `StoryAppTests` test target — the same D10 gap I18 and I25 already logged) that nobody has asked for yet. |
| 2026-08-13 | claude | **A rendered reel is now viewable without re-rendering it — requested directly right after the report caching above ("I should have the option to view the existing reel for the folder and don't have to remake it").** `RunRecord` gains `reelJobID: String?` (Optional, so a pre-existing persisted record with no such key still decodes — Swift's synthesized `Decodable` calls `decodeIfPresent` for `Optional` properties automatically); `ReelOptionsModel` gains an `onJobStarted: (@MainActor (String) -> Void)?` callback fired the moment `POST /trips/{id}/reel` returns a `job_id` — **queued, not finished**, so a reel started and then the app killed mid-render still has something to check next launch, matching the build job's own resumability story. `LoopScreen.reportSection` now offers three links once `jobID` for the reel exists: "View last reel" (new — `ReelViewerScreen`/`ReelViewerModel`, calling only `GET /jobs/{id}/reel` for a fresh signed URL against the same immutable, already-rendered video, never `POST`ing a new render) alongside "Make a new reel" (was "Make a reel" — relabelled once a prior reel exists, so the two links read as alternatives rather than one being a mystery duplicate of the other). Also fixed a latent bug while touching `saveRun`: it constructed a brand-new `RunRecord` on every call, which would have silently dropped `reelJobID` the very next time a build re-ran — now merges into the existing record instead. Builds clean for the physical device (`xcodebuild`); hostless `swift build` cannot check any of `LoopModel`'s or `ReelOptionsModel`'s changes, both sit entirely inside `#if os(iOS)`. Installed and relaunched. Not yet watched by a human starting a reel, killing the app mid-render, relaunching, and finding "View last reel" waiting — that full loop is the next real check, not just the individual pieces. |
| 2026-08-13 | claude | **Picking an already-built folder now shows its report instantly, no network call — requested directly ("so I don't have to re-send the folder everytime").** `fetchAndUnzipReport` already leaves its unzipped bundle on disk forever, keyed by job id (`Documents/report-<jobID>/`, never deleted) — the gap was that nothing ever looked there again. `loadRun(for:)`, called from both `restore()` (app relaunch) and `folderSelectionChanged(to:)` (picking a different folder in the picker), now calls a new `loadCachedReport(jobID:)` that checks for that directory and loads it straight into `reportBundle` with `phase = .ready` if it exists — silent on a miss, since `run()` (now labelled "Rebuild" instead of "Resume" once a cached bundle is showing, via `runButtonTitle`) is still there for a folder that was never sent or whose build failed. Also fixes a small pre-existing staleness bug while touching this code: `loadRun` did not clear `reportBundle`/`phase` on folder switch at all, so selecting a *different* folder with no run of its own could still show the *previous* folder's report section, sitting there stale until the next `run()` call overwrote it — now cleared unconditionally at the top of `loadRun`, same reasoning the folder-scoping fix from two days ago already applied to `tripID`/`jobID`. Verified: builds clean for the physical device (`xcodebuild`; hostless `swift build` cannot check this file's `LoopModel` at all — it sits entirely inside `#if os(iOS)`), installed and relaunched. Not yet watched by a human picking between two already-built folders and seeing each report load instantly — that's the next real check. |
| 2026-08-12 | claude | **Reel player had no sound and no fullscreen control, both reported immediately after the in-app player landed — same commit's oversight, fixed within the hour.** `ReelPlayerView` swapped SwiftUI's `VideoPlayer` for `AVPlayerViewController` directly via `UIViewControllerRepresentable`: (1) audio — the default `AVAudioSession` category (`.soloAmbient`) is silenced by the ringer/silent switch, so a video with a real audio track played with no sound and no error; `.playback` (set once, on player creation) is what every video app uses to be heard regardless of that switch. (2) fullscreen — `AVPlayerViewController`'s own control bar carries a native expand button; `VideoPlayer` gave no way to confirm or configure it, so wrapping the controller directly was the fix rather than searching for a SwiftUI-only affordance that may not exist. Builds clean both hostless (`swift build`, confirming the file outside `#if os(iOS)` is untouched) and for the physical device (`xcodebuild`, the only check that actually compiles this block); installed and relaunched. Not yet watched play with sound by a human — that confirmation is the next step. |
| 2026-08-12 | claude | **Reel screen's ready state now plays in-app, replacing the Open-in-Safari/share-sheet stand-in from a few minutes earlier — requested directly ("stream the reel video in app").** `ReelPlayerView` (`AVKit.VideoPlayer` over `AVPlayer(url:)`) streams the signed URL directly; `+faststart` on the mux (`9eb53ab`) is what makes that playable before the file has fully arrived, same premise I32 itself is built on. **This is not I32 and does not close it**: no `MediaCache` (I33) integration, no background download, so a second visit to this screen re-streams from the same signed URL rather than replaying a local file — I32's own criterion is specifically the caching half, left untouched and still `todo`. `ShareLink` kept alongside for saving/AirDrop; the `Link("Open in Safari", ...)` stand-in was removed since in-app playback supersedes it. Verified: builds clean under `xcodebuild` for the physical device (macOS `swift build` cannot check this — the `#if os(iOS)` half is invisible to it, the same repeated warning this file already carries), installed and launched; a human watching the video actually play is the next real check. |
| 2026-08-12 | claude | **I31 bug found on the first real device tap-through: the security-scope bracket opened too late.** `MusicImporter.importTrack` ran `DRMCheck.check(url:)` — which reads the file via `AVURLAsset.load` — *before* `pickedURL.startAccessingSecurityScopedResource()`, so on a real device the very first read of a `.fileImporter` URL failed with "no permission to open," reported directly ("app says i don't have permission to open the downloaded mp3 file"). The bracket now wraps the DRM check too, not just the copy that follows it. All 5 `MusicImportTests` still pass unchanged — they construct plain, non-scoped file URLs, so the bracket was always a no-op for them, which is exactly why this shipped past the existing suite and was only caught by a human's finger on real hardware. No test added: reproducing a genuinely security-scoped URL outside `UIDocumentPickerViewController`'s real UI is not something this suite can manufacture, the same limit I30-33's own log entry already named for `hasProtectedContent`'s positive path. |
| 2026-08-12 | claude | **I30/I33 wired into the app shell — built, tested, but unreachable is not the same as done, and now it's reachable.** `ReelOptionsScreen` (I30) existed as real, tested code with nowhere to tap it from; `HostApp` had two tabs and neither opened it. `LoopScreen.reportSection` now shows a second `NavigationLink("Make a reel")` beside "View the trip," gated on a new `LoopModel.makeReelOptionsModel()` that mirrors `makeAssetScheme()`'s own gate (`nil` until a trip exists) and hands `ReelOptionsModel` the same `endpoint`/`identity`/`tripID` the report path already resolved — no new state, no new persistence. **I32 (Playback) is not built and this does not build it**: instead of leaving a finished reel with a byte count and no way to watch it, the `.ready` phase now offers `Link("Open in Safari", ...)` and `ShareLink("Share or save", ...)` against the same signed `download.videoDownloadURL` — Safari/Photos play a progressive MP4 directly, and the share sheet covers AirDrop/Files/Save Video. Both are a stand-in and say so in a comment; neither touches `MediaCache` (I33) or implements I32's background-download-then-replay-locally criterion, so I32 stays `todo`. This was prompted directly ("i can't find the video reel") after the LAN IP fix (service restarted with `STORY_SERVICE_PUBLIC_BASE_URL` corrected from a stale `.81` to the current `.87` — DHCP had reassigned it, which is why "send" was hanging on upload with no error, just a connect to a host that no longer answered). Verified: `xcodebuild build -scheme HostApp` for the physical device succeeded, installed and launched via `devicectl` — a human tap-through of the reel path itself (options → render → open/share) is the next thing to actually watch happen, not yet done here. I30/I31/I33's tracker status is unchanged (`review`) — this is plumbing, not the device verification those rows are still waiting on. |
| 2026-08-12 | claude | **I18 done → review: a scope's `[PHAsset]` array is now something a human can see and prune, not just count.** `ThumbnailLoader` (`PhotoExport`) wraps `PHImageManager` behind an `async`, `NSCache`-backed call keyed by `identifier@WxH` — size is part of the key, not just `localIdentifier`, because the grid tier (300×300) and the detail tier (1200×1200) ask for the same asset at two different sizes, and a size-blind cache would hand a blown-up grid thumbnail to the full-screen page or vice versa. Real PhotoKit is reached through `ThumbnailImageProviding`, a one-method protocol (`PHImageManager` cannot be subclassed or driven off a real, authorized library per D10/D11), so the cache itself is proven with synthetic identifiers and a counting fake — no simulator needed for that half. `SelectionGridScreen` (`StoryApp`) is a `LazyVGrid` reading `ExportModel.assets` and writing straight into a new `ExportModel.excludedAssetIDs: Set<String>` via `Binding`, matching `ExportScreen`'s one-state-owner convention rather than inventing a second `@Observable`; a tap dims/toggles a cell, a long-press opens a bounded-size (1200pt, not original) full-screen page with the same toggle, and a toolbar menu offers select-all/deselect-all. `ExportModel.includedAssets` (assets minus exclusions) is now what `export()` actually sends, `findMedia()` clears the exclusion set on every fresh search, and the home-exclusion preview was moved from previewing against `assets` to previewing against `includedAssets` — a grid deselection already means "do not send this," so the number worth showing is what home-filter would *additionally* remove from what is actually about to export, made consistent rather than picking one silently. **Real verification, not assumed:** cleaned `DerivedData` and ran `xcodebuild test -project StoryBookHost.xcodeproj -scheme PhotoExportTests -destination 'platform=iOS Simulator,id=<iPhone 17 Pro>'` — **88 tests in 11 suites passed**, including 5 new `ThumbnailLoaderTests` on the device runtime itself: a cache hit suppresses a second provider call, a *fresh, cache-less* loader is the control that still reaches the provider (proving the cache, not the mock, is what suppressed the first case), two sizes for one identifier are cached separately, a miss is retried rather than cached as a permanent nil, and the literal "done when" line — 50 synthetic identifiers requested twice ("two scrolls") cost exactly 50 provider calls, not 100. `xcodebuild build -scheme HostApp` for the same simulator destination also succeeded (macOS `swift build` never compiles the `#if os(iOS)` half, same warning I30-33's log left), and I installed and launched the built `HostApp` on the booted simulator and screenshotted it — `ExportScreen` renders with no crash, before and after granting photo access and seeding 4 fixture images via `simctl addmedia`. **What is written but not run:** `ExportModelSelectionTests.swift` (`StoryAppTests`) is the literal folder-diff acceptance test the brief asked for — seeds two real fixtures via a small `SelectionSeeding` helper (duplicated from `PhotoExportTests/LibraryFixtures.swift` rather than importing it, since `Package.swift`, I01's contract, declares no cross-test-target dependency), deselects one, exports, asserts the written folder is missing exactly that file by name, reselects, exports again, asserts it is back — against a real `FolderWriter`, not the in-memory array. It compiles (confirmed indirectly: the same `PhotoExportTests` scheme build compiles all four package targets including `StoryAppTests`'s sibling files with no errors) but has never executed, because `StoryBookHost.xcodeproj` wires only the `PhotoExportTests` scheme (D10) — `StoryAppTests` has no scheme and thus no TCC bundle identity, the identical gap I25's log recorded for `PhotoKitAssetSourceTests`'s positive path. Not fixed here: wiring a scheme is an Xcode-project change bigger than this task's file ownership, and D10 already assigns that class of change deliberately rather than per-task. Root suite untouched (not run — Python side), hostless `swift test` still 226 (my new files are entirely `#if os(iOS)`-gated and compile to nothing on macOS, by design, matching `ThumbnailLoader`'s and `SelectionGridScreen`'s siblings). Set `review`, not `done`, for exactly that reason: the cache half of the acceptance criterion was watched pass on real simulator hardware, the folder-diff half was not. |
| 2026-08-12 | claude | **S06 done → review: `resolve_principal`'s body is real verification now, not a believed header.** `Authorization: Bearer <token>` routes on an unverified `iss` read against an exact two-entry allow-list (`https://accounts.google.com`, `accounts.google.com`, `https://appleid.apple.com`), then verifies for real via `jwt.PyJWKClient(...).get_signing_key_from_jwt` + `jwt.decode(algorithms=["RS256"], audience=..., issuer=...)` — one path for both providers, since both are RS256-over-JWKS. Identity is `sub` (never email — Apple only returns it on first login), fed into `index.ensure_user(kind="google"|"apple", value=sub)` unmodified from what S02 already anticipated (`IDENTITY_KINDS` already carried both kinds). `X-Story-Identity` still works for dev, now behind `Settings.allow_dev_identity_header` (default `True`; D14's promise that it flips to `False` before any non-localhost exposure is now an enforceable flag, not just prose). **The property that actually needed proving:** a bad bearer token never falls through to the dev header even when a valid one rides along on the same request — the downgrade attack this design exists to prevent. 14 new tests (service suite 226 → 240, `tests/test_principal.py`): both Google issuer forms, Apple, expired, wrong audience, unlisted issuer (plain and as a downgrade attempt), a tampered signature byte, an unconfigured provider (must not verify against a missing audience), both dev-header settings, and the literal S06 acceptance criterion re-run against **verified** tokens through a real `TestClient` — two `sub`s, account A 404s on B's trip and the list shows only its own — mirroring `test_ingest.py::TestPerUserIsolation`'s dev-header version. Only the JWKS network fetch is mocked; signature/audience/issuer/expiry checks run for real against a real RSA keypair and real `jwt.encode`/`decode`, no live network call. Root `tests/unit` untouched and green, `ruff` clean, `index.py`/`index_sqlite.py`/every route file untouched. **Set `review`, not `done`:** no live Google/Apple token from a real client has exercised this (needs a provisioned client id and I23's device half), and open question 3 (account deletion vs. indefinite retention) is still unresolved — blocks Apple App Store submission, not landing this code. Not marking I23 unblocked myself; that's the next owner's call, but S06 landing is what unblocks it. |
