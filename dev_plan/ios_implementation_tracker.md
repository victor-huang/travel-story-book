...
| 16 | **What is the queue, and does the worker run the same image?** S03 needs "one worker per trip at a time" and progress read from `stage_result`, so the worker wants the same filesystem the build wrote to. That is a hosting property, not a library choice. <br><br>**PROPOSED 2026-08-10 by S03, awaiting ratification — implemented, and reversible.** A **`job` table in the relational index**, claimed in one `BEGIN IMMEDIATE` transaction, polled by a worker that runs the CLI as a subprocess. **Yes, the same image**: the worker is a thread in the API process by default (`STORY_SERVICE_WORKER_INLINE=0` turns it off) and `python -m storybook_service.worker` runs it beside the API from that same image; the claim transaction makes both identical. Inline is the default because the other default fails *silently* — a queued job nothing picks up, on a service whose `/health` says `ok`. **"One worker per trip" is a partial unique index** (`state IN ('queued','running')`), so the bad state is unrepresentable rather than defended against. <br><br>*Why not a broker.* Redis/Celery or SQS each add a service to operate and a second store that can disagree with the index; SQS additionally delivers at-least-once, which for a multi-hour build means designing against duplicate execution rather than preventing it. Neither buys anything usable here, because the worker must see the same filesystem the build wrote to — progress is read from that trip's `story.db`, and a resumed build reuses `--out`. <br><br>**What a human must accept: this expires the moment a second instance exists**, exactly as question 19's answer says of the index. Two API containers behind a load balancer are fine (turn the inline worker off on both and run one worker). A second *worker*, on another machine, is not — the trip directory is local, and **nothing in the code would detect that mistake**. If the answer is "we will scale out", the queue and the storage layout change together, not just the queue. | S03 |
...
| 18 | **Does the deployed image carry the `clip` extra?** Torch multiplies the image size and the cost; without it `EmbeddingStage` is unavailable and dedup loses its semantic half, which is a visibly worse result rather than a failure. `service/`'s dependencies exclude it today and `/ready` reports its absence by name, so nothing silently claims embeddings ran. <br><br>**Still a cost question for the human, and S03 has made it safe to answer either way.** Every job now records, at its own start, which stages this deployment could run — by calling each stage's `available()` and keeping the stage's own words — so `GET /jobs/{id}` returns `degraded: true` and `unavailable_stages: [{stage, reason, affects}]`. A `clip`-less build therefore reports *"CLIP unavailable: missing torch, open_clip; install the 'vision' extra"* against `embeddings` **and** `content_class`, which is worth knowing: without CLIP the pipeline loses not only semantic dedup but the screenshot/receipt/document classifier, so the proposal grid will show things a traveller would have expected to be filtered out. **S03's recommendation: ship without `clip` first and watch a real trip's dedup and proposal quality**, because the degradation is now legible per job rather than inferred, and adding the extra later changes an image, not a contract. What must not happen is shipping without it *and* without this reporting. | S03 |
